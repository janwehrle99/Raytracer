...
bool Sphere<FLOAT,N>::intersects(Sphere<FLOAT, N> sphere) const {
  ...
  // Abstand von einem Center zum anderen Center berechnen
  // Wenn dieser Abstand < als die Summe beider radien dann überschneiden sie sich

  float sum = 0;
  for (size_t i = 0; i < N; i++) {
    sum += pow((this->center[i]-sphere.center[i]), 2);
  } 
